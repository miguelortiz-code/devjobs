...
const addVacancy = async (req, res) => {
  ...
  const { title, company, ubication, salary, contract, description, skills } =
    req.body;
  const { _id, name } = req.user;
  // Validar y sanitizar campos
  await check("title")
    .notEmpty()
    .withMessage("El nombre la vacante es obligatorio")
    .trim()
    .escape()
    .toLowerCase()
    .run(req);
  ...
  await check("contract")
    .notEmpty()
    .withMessage("Debes seleccionar un tipo de contrato")
    .trim()
    .escape()
    .toLowerCase()
    .run(req);
  ...
  await check("skills")
    .notEmpty()
    .withMessage("Debes seleccionar al menos una habilidad")
    .trim()
    .escape()
    .toLowerCase()
    .run(req);
  ...
  try {
    ...
    if (!result.isEmpty()) {
      ...
      const selectedSkills =
        typeof skills === "string" ? skills.split(",") : vacancy.skills;
...
const uploadResume = async (req, res, next) => {
  ...
  try {
    ...
    // Si existe la vacante, crear el objeto de los candidatos
    const newCandidate = {
      name, email, cv };

    // Almacenar al candidato
    vacancy.candidates.push(newCandidate);
    await vacancy.save();
    // Mensaje y redireccionar
    req.flash("correcto", "Se ha enviado tu curriculum correctamente");
    return res.redirect("/");
  } catch (error) {
    console.log(`Error la subir el CV: ${error}`);
    req.flash("error", "Hubo un error al enviar tu CV");
